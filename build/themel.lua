...
bd = setmetatable({
    ...
    __index = function(self, i)
        rawset(self, i, i:find('Service') and Knit.GetService(i) or Knit.GetController(i))
...
local Combat = Window:AddTab("Main", "skull")
local CB = Combat:AddLeftTabbox("Combat")
local CT = CB:AddTab("Combat")

local function GetTool()
    return lplr.Character and lplr.Character:FindFirstChildWhichIsA('Tool', true) or nil
end

local function IsInFOV(targetPos, angle)
    local camDir = gameCamera.CFrame.LookVector
    local toTarget = (targetPos - gameCamera.CFrame.Position).Unit
    local deg = math.deg(math.acos(camDir:Dot(toTarget)))
    return deg <= angle
end

local function GetClosestTarget(range, fov)
    local closest, closestDist = nil, range
    for _, v in pairs(playersService:GetPlayers()) do
        if v ~= lplr and v.Character and v.Character:FindFirstChild("HumanoidRootPart") and v.Character:FindFirstChild("Humanoid") and v.Character.Humanoid.Health > 0 then
            local pos = v.Character.HumanoidRootPart.Position
            local dist = (pos - lplr.Character.HumanoidRootPart.Position).Magnitude
            if dist <= closestDist and IsInFOV(pos, fov) then
                closest = v
                closestDist = dist
            end
        end
...
local SwingDelay = 0
local infJump, infJumpDebounce

local Lel = CT:AddSlider("ReachSlider", {
    Text = "Range",
    Default = 14,
    Min = 1,
    Max = 50,
    Rounding = 1,
    Callback = function(v) RangeKa = v end
})

local AngleSlider = CT:AddSlider("FOVSlider", {
    Text = "Attack Angle (FOV)",
    Default = 90,
    Min = 1,
    Max = 360,
    Rounding = 1,
    Callback = function(v) KaAngle = v end
})

local KA = CT:AddToggle("KillauraToggle", {
    Text = "Killaura",
    Default = false,
    Tooltip = "Auto attack closest target",
    Callback = function(v)
        KillAuraRunning = v
        if v then
            task.spawn(function()
                while KillAuraRunning do
                    local tool = GetTool()
                    if tool and tool:HasTag("Sword") and lplr.Character and lplr.Character:FindFirstChild("HumanoidRootPart") then
                        local target = GetClosestTarget(RangeKa, KaAngle)
                        if target then
                            local ent = bd.Entity.FindByCharacter(target.Character)
                            if ent then
...
local Rc = CT:AddSlider("Reach", {
    ...
    Callback = function(v)
        ReachRange = v
        oldReach = rawget(bd.CombatConstants, 'REACH_IN_STUDS')
        rawset(bd.CombatConstants, 'REACH_IN_STUDS', ReachRange)
        rawset(bd.Entity.LocalEntity, 'Reach', ReachRange)
    end
})

local RH = CT:AddToggle("ReachToggle", {
    Text = "Reach",
    Default = false,
    Tooltip = "Extend hit reach",
    Callback = function(v)
        if not v and oldReach then
            rawset(bd.CombatConstants, 'REACH_IN_STUDS', oldReach)
            rawset(bd.Entity.LocalEntity, 'Reach', oldReach)
            oldReach = nil
        end
    end
})
...
local critHook
local Crit = CT:AddToggle("AutoCrit", {
    Text = "Auto Critical",
    Default = false,
    Tooltip = "Forces critical attacks",
    Callback = function(v)
        if v then
            critHook = hookfunction(bd.Blink.item_action.attack_entity.fire, function(...)
                local data = ...
            ...
            if critHook then
                hookfunction(bd.Blink.item_action.attack_entity.fire, critHook)
                critHook = nil
            end
        end
    end
...
local AntiSlowOld = {}
local AntiSlow = CT:AddToggle("AntiSlow", {
    Text = "Anti Slow",
    Default = false,
    Tooltip = "Bypasses movement slowing",
    Callback = function(callback)
        local func = debug.getproto(bd.MovementController.KnitStart, 5)
        if callback then
            AntiSlowOld = debug.getconstants(func)
            for i, v in ipairs(AntiSlowOld) do
            ...
            table.clear(AntiSlowOld)
        end
    end
})

local InfJump = CT:AddToggle("InfJump", {
    Text = "Infinite Jump",
    Default = false,
